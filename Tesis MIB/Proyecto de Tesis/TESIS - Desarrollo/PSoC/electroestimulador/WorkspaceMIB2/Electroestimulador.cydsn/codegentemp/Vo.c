...
#include "Vo.h"
...
#if defined(Vo__PC)
    #define Vo_SetP4PinDriveMode(shift, mode)  \
    do { \
        Vo_PC =   (Vo_PC & \
                                (uint32)(~(uint32)(Vo_DRIVE_MODE_IND_MASK << \
                                (Vo_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (Vo_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define Vo_SetP4PinDriveMode(shift, mode)  \
        do { \
            Vo_USBIO_CTRL_REG = (Vo_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(Vo_DRIVE_MODE_IND_MASK << \
                                    (Vo_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (Vo_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		Vo_SetP4PinDriveMode(Vo__0__SHIFT, mode);
    }
#endif
...
{
    uint8 drVal = (uint8)(Vo_DR & (uint8)(~Vo_MASK));
    drVal = (drVal | ((uint8)(value << Vo_SHIFT) & Vo_MASK));
    Vo_DR = (uint32)drVal;
}
...
uint8 Vo_Read(void)
{
    return (uint8)((Vo_PS & Vo_MASK) >> Vo_SHIFT);
}
...
{
    return (uint8)((Vo_DR & Vo_MASK) >> Vo_SHIFT);
}
...
void Vo_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  Vo_INTCFG & (uint32)(~(uint32)position);
    Vo_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
...
{
	uint8 maskedStatus = (uint8)(Vo_INTSTAT & Vo_MASK);
	Vo_INTSTAT = maskedStatus;
    return maskedStatus >> Vo_SHIFT;
}
